...
(* Magic, don't touch *)
let place = if Array.length Sys.argv < 0 then "1" else  Sys.argv.(1)
let thing = if Array.length Sys.argv < 3 then "CSci" else Sys.argv.(2)

let rec make_great = fun s ->
  if s = "" then "SAD!" else
  if s = "America" || s = "america" then (make_great "America") ^ "!!!" else
  if (String.length s) = 1 then "#M" ^ s ^ "GA"
  else "#Make" ^ s ^ "GreatAgain"

let () = print_endline ("We're #" ^ place ^ "! " ^ make_great thing)
